...
export const createDiff = (
  ...
  const attUpdater: AttributesUpdater = {
    remove: Object.keys(oldNode.props || {}).filter(
      (att) => Object.keys(newNode).indexOf(att) == -1
    ),
  ...
  const childsUpdater: ChildUpdater[] = childsDiff(
    oldNode.childeren || [],
    newNode.childeren || []
  );
...
const childsDiff = (
  ...
  const remainingOldChilds: [string | number, VDomNode][] = oldChilds.map(
    (c) => [c.key, c]
  );
  const remainingNewChilds: [string | number, VDomNode][] = newChilds.map(
    (c) => [c.key, c]
  );
  ...
  let [nextUpdateKey] = remainingOldChilds.find(
    (k) => remainingNewChilds.map((k) => k[0]).indexOf(k[0]) != -1
  ) || [null];
  ...
  while (nextUpdateKey) {
    ...
    // create the update
    operations.push(
      createDiff(remainingOldChilds.shift()[1], remainingNewChilds.shift()[1])
    );

    // find the next update
    [nextUpdateKey] = remainingOldChilds.find(
      (k) => remainingNewChilds.map((k) => k[0]).indexOf(k[0]) != -1
    ) || [null];
  }
